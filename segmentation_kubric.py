...
def load_rgbd_cam(vid: str, root_dir: str, npz_folder: str,  hfov: float, new_imw=1, new_imh=1):
  """load rgb, depth, and camera"""
  input_dict = {'left': {'camera': [], 'depth': [], 'video': []}}
  # Load camera
  depths, rgbs, intrinsics_normal, intrinsics, extrinsics, cam_ID = read_cam_kubric(root_dir, int(vid.split('_')[-1]), depth_euclid = False, query_points = None)
  nfr = len(depths)
  input_dict['nfr'] = nfr
  for fid in range(nfr):
    intr_normalized = {
        'fx': intrinsics_normal[0,0],
        'fy': -1*intrinsics_normal[1,1],
        'cx': 0.5,
        'cy': 0.5,
        'k1': 0,
        'k2': 0,
...
def segmentation_main(vid: str, save_root: str, npz_folder: str, hfov: float):
  ...
  input_dict = load_rgbd_cam(vid, save_root, npz_folder, hfov)

...
def read_dexycb_rgb(view_path:str):
    
  video = read_images_from_directory(dir = os.path.join(view_path, 'rgb'), prefix = None)
...
def run_track(vid: str, save_root: str):
  ...
  """
  # Read the video.
  video = read_dexycb_rgb(osp.join(save_root, vid))
...
def main():
  ...
  for idx in views:
    ...
    run_track(f'view_{idx:02d}', args.dir)

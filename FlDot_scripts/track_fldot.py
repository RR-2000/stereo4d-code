...
def read_fldot_rgb(sequence:str, cam_ID:int):
    
  video = read_images_from_directory(os.path.join(sequence, f'images', 'scaled_view'), idx = cam_ID+1, prefix=None) # images are 1-indexed in the directory
  
  return video[..., :3]


def run_track(vid: int, save_root: str):
  """
  ...
  """
  # Read the video.
  video = read_fldot_rgb(save_root, vid)
  vid = f'view-{vid:02d}'
  os.makedirs(osp.join(save_root, vid), exist_ok=True)

...
def load_rgbd_cam(vid: str, root_dir: str, npz_folder: str,  hfov: float, new_imw=1, new_imh=1, depth: str = 'gt', num_views: int = 8):
  ...
  # Load camera
  depths, rgbs, intrinsics_normal, intrinsics, extrinsics, cam_ID = read_cam_dex(root_dir, int(vid.split('-')[-1]), query_points = None, depth = depth, num_cams=num_views)
  nfr = len(depths)
  input_dict['nfr'] = nfr
...
def plot_3d_tracks_with_camera(track3d: utils.Track3d, depth, rgb, axes=None, tracks_leave_trace=32):
  ...
  for t in tqdm.tqdm(range(num_frames), desc='plotting tracks'):
    fig = Figure(figsize=(6.40, 5.12), dpi=100)
...
def optimize_track_main(vid: int, save_root: str, npz_folder: str, hfov: float, depth: str = 'gt', num_views: int = 8):
...
def main():
  ...
  if args.optimize:
    ...
    for idx in tqdm.tqdm(views):
      optimize_track_main(idx, args.dir, args.dir, args.hfov,args.depth, args.num_views)

  full_tracks = []
  for idx in views:
    path = osp.join(args.dir, f'view-{idx:02d}', f'view-{idx:02d}' + '-optimized_tracks.pkl')
    if REG == 'None':

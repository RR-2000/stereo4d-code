...
def load_rgbd_cam(vid: str, root_dir: str, npz_folder: str,  hfov: float, new_imw=1, new_imh=1, depth: str = 'gt', num_views: int = 8):
  ...
  # Load camera
  depths, rgbs, intrinsics_normal, intrinsics, extrinsics, cam_ID = read_cam_arctic(root_dir, int(vid.split('_')[-1]), query_points = None, depth = depth, num_cams=num_views)
  nfr = len(depths)
  input_dict['nfr'] = nfr
...
def save_3d_track_vis(
  ...
  track3d_dynamic = track3d.get_new_track((motion_mag > 0).any(axis=1))
  ...
  masks = np.ones(len(track3d_dynamic_sort.track3d))
...
def plot_3d_tracks_with_camera(track3d: utils.Track3d, depth, rgb, axes=None, tracks_leave_trace=32):
  ...
  for t in tqdm.tqdm(range(num_frames), desc='plotting tracks'):
    fig = Figure(figsize=(20.00, 28.00), dpi=100)
...
def save_track_permutations(args):
  """
  Save permutations of track combinations based on the number of cameras.
  This function creates different track combinations by taking subsets of the available views.
  
  Args:
    args: The command-line arguments containing directory and number of views.
  """
  
  print(f"Generating track permutations for {args.num_views} cameras...")
  
  # Define the permutation sizes to generate
  sizes = [1, 2, 3, 4, 6, 8, 12, 16, 20, 24, 32] if args.num_views >= 32 else [1, 2, 3, 4, 6, 8, 12, 16, 20]
  sizes = [s for s in sizes if s <= args.num_views]
  
  # Create output directory
  perm_dir = Path(args.dir) / "permutations"
  ...
  for idx in range(args.num_views):
    ...
    try:
      with open(path, 'rb') as f:
        arr = pickle.load(f)
        all_tracks.append((idx, arr['track3d']))
    except FileNotFoundError:
      print(f"Warning: Could not find tracks for view_{idx}")
  
  # Generate permutations for each size
  for size in sizes:
    print(f"Generating permutations of size {size}...")
    
    # Generate combinations (not all permutations to avoid excessive combinations)
    # For larger sizes, take a limited number of combinations
    all_combinations = list(itertools.combinations(range(args.num_views), size))
    max_combinations = 10  # Limit number of combinations to save
    
    if len(all_combinations) > max_combinations:
      # Choose evenly spaced combinations
    ...
    for i, combo in enumerate(combinations):
      ...
      # Collect tracks for this combination
      for idx in combo:
        for view_idx, tracks in all_tracks:
          if view_idx == idx:
            combo_tracks.append(tracks)
            combo_indices.append(idx)
            break
      
      # If we found tracks for all views in the combination
      if len(combo_tracks) == len(combo):
        # Concatenate tracks and filter invalid ones
        combined_tracks = np.concatenate(combo_tracks, axis=0)
        invalid = np.any(np.isnan(combined_tracks.reshape(combined_tracks.shape[0], -1)), axis=1)
        filtered_tracks = combined_tracks[~invalid].transpose(1, 0, 2)
        
        # Save the combination
        combo_name = f"tracks_{size}cams_{'_'.join(map(str, combo_indices))}"
        save_path = perm_dir / f"{combo_name}.npy"
        print(f"Saving {combo_name} with shape {filtered_tracks.shape}")
        np.save(save_path, filtered_tracks)
  
  print(f"Saved all track permutations to {perm_dir}")
...
def main():
  ...
  parser.add_argument('--dir', help='path to views', type=str, default='/project/Thesis/data/arctic_data/images/s01/box_grab_01')
  ...
  parser.add_argument('--exp', help='experiment name', type=str, default='')
  ...
  if args.optimize:
    ...
    for idx in tqdm.tqdm(views):
      optimize_track_main(f'{idx}', args.dir, args.dir, args.hfov,args.depth, args.num_views)
  ...
  if args.filter:
    static_points = np.linalg.norm(full_tracks[0] - full_tracks[-1], axis=-1) < 0.01
    print(static_points.shape)
    print(static_points.mean())
    print(static_points.sum())
    full_tracks = full_tracks[:, ~static_points]

  file_name = f'stereo4d_tracks_{args.noise}_{args.exp}' if args.noise is not None else f'stereo4d_track_{args.exp}'

  print(f'Saving combined tracks to { osp.join(args.dir,  file_name + ".npy")} with shape {full_tracks.shape}')
  # save_track_permutations(args)

  np.save(osp.join(args.dir, file_name),full_tracks)

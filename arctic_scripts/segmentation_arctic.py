...
def load_rgbd_cam(vid: str, root_dir: str, npz_folder: str,  hfov: float, new_imw=1, new_imh=1, depth: str = 'gt', num_cams: int = 8):
  ...
  # Load camera
  depths, rgbs, intrinsics_normal, intrinsics, extrinsics, cam_ID = read_cam_arctic(root_dir, int(vid.split('_')[-1]), query_points = None, depth = depth, num_cams=num_cams)
  nfr = len(depths)
  input_dict['nfr'] = nfr
...
def segmentation_main(vid: str, save_root: str, npz_folder: str, hfov: float, depth: str = 'gt', num_cams: int = 8):
  ...
  video = read_images_from_directory(dir = osp.join(save_root, vid), prefix = None)[..., :3]
  seg_maps = []
...
def main():
  ...
  for idx in views:
    ...
    segmentation_main(f'{idx}', args.dir, args.dir, args.hfov, args.depth, args.num_views)

...
def read_images_from_directory(dir :str, idx :int, prefix = None, num_frames = 40):
    """
    ...
    cam_ID = f'{idx:03d}' # Format camera ID as 3-digit
    images = []
    if prefix is None:
        for filename in sorted(os.listdir(dir)):
            filepath = os.path.join(dir, filename)
            if os.path.isfile(filepath) and filename.lower().endswith((cam_ID+'.png', cam_ID+'.jpg', cam_ID+'.jpeg')) and int(filepath.split('/')[-1][5:11]) <= num_frames*6:
...
def read_cam_dex(sequence :str, idx :int, query_points = None, depth: str = 'dust3r', num_cams: int = 5):
    """
    Reads camera and depth data for a given sequence and view index.
    Selects the appropriate depth type: 'gt', 'vggt', or 'dust3r'.
    """
    if depth == 'dust3r':
        return read_cam_fldot_dust3r(sequence, idx, query_points, num_cams=num_cams)
    else:
        raise ValueError(f"Unknown depth type: {depth}. Use 'dust3r'.")
    
def get_cam_info(calib_file, idx=1):
    """
    ...
    with open(calib_file, 'r') as f:
        ...
        if line.strip():
            values = line.split(' ')
            values = [float(x) for x in values[1:-1]] # removing \n and cam_ID
            intr = values[0:5]
            # fu u0 v0 ar s
            cam_info['intrinsics']= np.asarray([[intr[0], intr[4], intr[1]],
                                                        [0, intr[0]*intr[3], intr[2]*intr[3]],
                                                        [0, 0, 1]])
            rot_qot = np.asarray(values[10:-5])
            rot_matrix = quaternion_to_rotation_matrix(rot_qot)
            transl = values[-5:-2]

            # UNCOMMENT TO DEBUD
            # a = ['fu', 'u0', 'v0', 'ar','s', 'k1', 'k2', 'p1', 'p2', 'k3', 'quaternion_s', 'quaternion_x', 'quaternion_y', 'quaternion_z', 'translation_x', 'translation_y', 'translation_z', 'width' ,'height']
            # for i in range(len(values)):
            #     print(f'{a[i]}: {values[i]}')
            
            # Create 4x4 extrinsics matrix [R|t; 0 0 0 1]
            extr_matrix = np.eye(4)
            extr_matrix[:3, :3] = rot_matrix
            extr_matrix[:3, 3] = np.array(transl, dtype=float)/SCENE_SCALE
            # extr_matrix = np.linalg.inv(extr_matrix)  # Invert to get camera to world transformation
            cam_info['extrinsics'] = (extr_matrix)

    return cam_info

def read_cam_fldot_dust3r(sequence :str, idx :int, depth_euclid = False, query_points = None, num_cams = 8):
    """
    ...
    cam_ID = idx
    meta = get_cam_info(os.path.join(sequence, 'cam_info', 'calib_info_scaled.txt'), idx=cam_ID +1)  # idx + 1 because cam_ID is 1-indexed
    ...
    imgs = read_images_from_directory(os.path.join(sequence, f'images', 'scaled_view'), idx = cam_ID+1, prefix=None)[..., :3] # images are 1-indexed in the directory
    h, w = imgs.shape[1:3]
